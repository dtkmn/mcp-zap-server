...
- the product boundary is the MCP-native gateway core, not ZAP itself
- extensions plug into public core contracts
...
The core is the part that should stay useful and credible for every OSS user.
It owns:
...
The core must run without enterprise installed. If removing an extension breaks
the basic gateway, the boundary is wrong.

## What An Extension Is

An extension is a provider that depends on core contracts and changes behavior
...
- adds future engine adapter behavior once the engine adapter ADR is complete

Extensions should not fork the core runtime or require MCP clients to learn a
different product.

```mermaid
flowchart TD
  Client["MCP client"]
  Core["OSS gateway core<br/>MCP tools and public contracts"]
...
describe current or future core boundaries, not a stable standalone-builder API.
...
| Requester identity and access | `WorkspaceIdentityResolver`, `ScanJobAccessBoundary`, `ReportArtifactBoundary`, `PolicyBundleAccessBoundary`, `ScanHistoryAccessBoundary` | Add tenant/workspace-aware visibility and access checks around shared core data. |
...
These are contracts, not permission to add private logic to core. If the
extension behavior is not public-safe, keep it outside the OSS export.

## Enterprise As A Private Extension Pack

Enterprise should be implemented as a private extension pack on top of the same
core. That is the point of the model.
...
core runtime. The standalone sample shows the external project shape that
...
until they have safe public view types. Engine adapter contracts should wait
until the engine extension ADR is complete.
...
- a full second scanner engine before the adapter ADR is complete
...
- core can still boot and pass tests without it
- the extension depends on core, not the other way around
...
Additional scanner engines should come through an adapter boundary, but that
boundary needs an ADR before implementation. Until then, engine work should be
limited to documenting roles, mapping the current ZAP interfaces, and fencing
unsupported claims.
...
That means an operator can build a runtime image that includes the core gateway
and an extension JAR on the application classpath. This is intentionally simpler
than a marketplace or dynamic plugin loader.
