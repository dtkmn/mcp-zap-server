...
editUrl: false
description: "Complete configuration examples for all three authentication modes."
---
Complete configuration examples for all three authentication modes.

Versioned container examples target `v0.13.0`. Check
...
services:
  mcp-zap-server:
    image: ghcr.io/dtkmn/mcp-zap-server:v0.13.0
    environment:
      - MCP_SECURITY_MODE=api-key
