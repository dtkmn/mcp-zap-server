...
public class ExpertAutomationMcpToolsService implements ExpertToolGroup {
    ...
    public String runAutomationPlan(
            @ToolParam(description = "Existing plan file path under the automation workspace (optional; provide either this or planYaml)") String planPath,
            @ToolParam(description = "Inline automation plan YAML content to materialize and run (optional; provide either this or planPath)") String planYaml,
            @ToolParam(description = "Optional file name to use when planYaml is provided (default: automation-plan.yaml)") String planFileName
    ) {
    ...
    public String getAutomationPlanStatus(
            ...
            @ToolParam(description = "Optional maximum number of info/warn/error messages to render (default: 20, max: 100)") Integer maxMessages
    ) {
    ...
    public String getAutomationPlanArtifacts(
            ...
            @ToolParam(description = "Optional maximum number of discovered artifact files to render (default: 20, max: 50)") Integer maxArtifacts,
            @ToolParam(description = "Optional total preview character budget across text artifacts (default: 8000, max: 50000)") Integer maxChars
    ) {
